...
class MultiHeadCompat(nn.Module):
    ...
    def forward(self, q, h = None, mask=None):
        ...
        # Calculate compatibility (n_heads, batch_size, n_query, graph_size)
        compatibility_s2n = self.kernel_func(Q, K)
...
class CompatNeighbour(nn.Module):
    ...
    def forward(self, h, rec, visited_order_map, selection_sig):
        ...
        compatibility = (self.kernel_func(Q_pre, hidden_K).sum(-1) + self.kernel_func(hidden_Q, K_post).sum(-1) - self.kernel_func(Q_pre, K_post).sum(-1))[:,:,1:]
